...
var startCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		tui.Start(time.Duration(duration)*time.Minute, task, os.Stdout, "#3005F2", "#7256F2", false)
	},
}

func init() {
	rootCmd.AddCommand(startCmd)
	// startCmd.Flags().StringVar(&color, "color", "#3005F2", "#7256F2", "Sets the color of the progress bar")
}
